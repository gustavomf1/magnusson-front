...
import { salvarRegraCashback } from '@/services/cashback'
...
export function ProductForm({ produto }: Props) {
  ...
  async function salvar() {
    ...
    try {
      const body = {
        ...
        descricaoSeo: form.descricaoSeo,
      }
  ...
  async function salvarCashback() {
    if (!produtoId) return
    const percentual = Number(form.cashbackPercentual)
    if (!percentual || percentual <= 0 || percentual > 100) {
      alert('Informe um percentual válido entre 0 e 100.')
      return
    }
    if (produto?.regraCashback) {
      const ok = confirm(
        'Já existe uma regra de cashback para este produto. Deseja substituí-la pelos novos valores?'
      )
      if (!ok) return
    }
    const prazo =
      form.cashbackPrazoValidadeDias.trim() === '' ? null : Number(form.cashbackPrazoValidadeDias)
    await salvarRegraCashback(produtoId, {
      percentual,
      prazoValidadeDias: prazo,
    })
    alert('Regra de cashback salva.')
  }

  const inputClass =
    'w-full border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-navy'
      ...
      <section>
        ...
        {!produtoId && (
          <p className="mt-2 text-xs text-gray-500">
            Salve o produto primeiro para gerenciar imagens, cores e tamanhos.
          </p>
        )}
      </section>
      ...
      {produtoId && (
        <>
          ...
          <section>
            ...
            <div className="flex gap-2 items-end">
              ...
              <button
                ...
                className="bg-gray-800 text-white px-3 py-1 text-sm h-9"
              >
                + Cor
              </button>
            </div>
          ...
          <section className="space-y-3 border-t border-gray-200 pt-6">
            <h2 className="font-cinzel text-lg text-navy">Cashback</h2>
            <p className="text-sm text-gray-600">
              Configure o percentual de cashback que este produto gera em cupom para o cliente
              quando o pedido é pago.
            </p>
            <div className="flex gap-4">
              <label className="flex-1 text-xs font-medium text-gray-600 uppercase tracking-wider">
                Percentual (%)
                <input
                  type="number"
                  min="0.01"
                  max="100"
                  step="0.01"
                  value={form.cashbackPercentual}
                  onChange={(e) => setForm((f) => ({ ...f, cashbackPercentual: e.target.value }))}
                  className="mt-1 w-full border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-navy"
                />
              </label>
              <label className="flex-1 text-xs font-medium text-gray-600 uppercase tracking-wider">
                Validade do cupom (dias, opcional)
                <input
                  type="number"
                  min="1"
                  step="1"
                  value={form.cashbackPrazoValidadeDias}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, cashbackPrazoValidadeDias: e.target.value }))
                  }
                  className="mt-1 w-full border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-navy"
                />
              </label>
            ...
            <button
              type="button"
              onClick={salvarCashback}
              className="bg-forest text-white px-4 py-2 text-sm tracking-wider hover:opacity-90"
            >
              Salvar cashback
            </button>
          </section>
